 ...
 */
typedef struct NeuronOperationMeta {
    /** The group ID set to the operation. */
    int64_t groupId;
    /** The core ID set to the operation. */
    int64_t coreId;
    /** The execution plan set to the operation. */
    const char* executionPlan;
} NeuronOperationMeta;
...
typedef enum {
     ...
     * YUV420_10BITS formats. AFBC (Arm Frame Buffer Compression) is a lossless compressed
     * image format, created by ARM to reduce the size of images.
     *
     * For input part, need to set two inputs with different shape, representing Y
     ...
     * channel size should be 1; for UV plane, channel size should be 2. Besides,
     * the height and width of UV plane should be half of Y's height and width.
     * For AFBC output, its height should be 3/2 of Y's height, and its width equals to Y's
     * width.
     *
     * Example:
     *
     ...
     * YUV420_8BITS. Set to 1 for YUV420_10BITS. Note that 8b, 10b here means the compressed
     * bit width in AFBC frame, where the YUV420 must be 8b for AFBC_8b, and must be 16b for
     * AFBC_10b.
     ...
     * to an input tensor element-wise.
     *
     * Supported tensor {@link OperandCode}:
     * * {@link NEURON_TENSOR_FLOAT16}
     * * {@link NEURON_TENSOR_FLOAT32}
     * * {@link NEURON_TENSOR_QUANT8_ASYMM}
     * * {@link NEURON_TENSOR_QUANT8_ASYMM_SIGNED}
     *
     * Tensor rank: Up to 4D, with "NHWC" data layout.
     *
     * Inputs:
     ...
     *      For a {@link NEURON_TENSOR_QUANT8_ASYMM} and
     *      {@link NEURON_TENSOR_QUANT8_ASYMM_SIGNED} tensor,
     *      the scale and zeroPoint must be the same as input0.
     *
     * Available since NeuroPilot 8.0.0.
     */
    NEURON_GELU = 108,
    /**
     * Applies the Gaussian Error Linear Unit (GELU) activation function
     ...
     *       A value indicating the approximate method:
     *        - 0: Sigmoid;
     *        - 1: Tanh;
     *        - 2: Erf;
     *
    ...
    /**
     * Decompress AFBC to YUV frame, support YUV444, YUV420_NV12 and
     * YUV420_P010 formats. Also, support YUV_8 and YUV_10 as bit width.
     * AFBC (Arm Frame Buffer Compression) is a lossless compressed image
     * format, created by ARM to reduce the size of images.
     *
     * For input part, need to set two inputs with different shape, representing Y
     * and UV plane respectively. For output part, need to set one output for YUV.
     *
     * The shape of the two inputs/ outputs (inputY, inputUV, outputY, outputUV)
     * depends on the original images' shape ([batches, height, width, channels]).
     * Both height and width shold follow 64 alignment rule. For example, if
     ...
     * If image mode is YUV420_8BITS, use NEURON_TENSOR_QUANT8_ASYMM; if mode is
     * YUV420_10BITS, use NEURON_TENSOR_QUANT16_ASYMM.
     * Also, if image mode is YUV444, NEURON_TENSOR_QUANT8_ASYMM for output0,
     * and no need to set output1
     *
     * Tensor rank: both input and output require rank 4, with "NHWC" data layout.
     *
     * Inputs:
     ...
     * * 6: yOffset, an {@link NEURON_INT32} scalar, specifying the frame
     * height offset of AFBC format.
     * * 7: yuvMode, an {@link NEURON_INT32} scalar. Set to 0 for
     * YUV444. Set to 1 for YUV420_NV12. Set to 2 for YUV420_P010.
     * * 8: bitWidth, an {@link NEURON_INT32} scalar. Set to 0 for
     * YUV_8. Set to 1 for YUV_10.
     * * 9: inPitchN, an {@link NEURON_INT32} scalar, specifying the
     * YUV N-axis pitch.
     * * 10: inPitchH, an {@link NEURON_INT32} scalar, specifying the
     * YUV H-axis pitch.
     * * 11: inPitchW, an {@link NEURON_INT32} scalar, specifying the
     * YUV W-axis pitch.
     * * 12: inPitchC, an {@link NEURON_INT32} scalar, specifying the
     * YUV C-axis pitch.
     *
     * Outputs:
     * * 0: output, a 4-D tensor.
     * For YUV444, Tensor type should be {@link NEURON_TENSOR_QUANT8_ASYMM}.
     * For YUV420, Tensor type can be {@link NEURON_TENSOR_QUANT8_ASYMM} or
     * {@link NEURON_TENSOR_QUANT16_ASYMM}, depends on YUV420 bit mode.
     * * 1: output, a 4-D tensor. Only for YUV420.
     * Tensor type can be either {@link NEURON_TENSOR_QUANT8_ASYMM} or
     * {@link NEURON_TENSOR_QUANT16_ASYMM}, depends on YUV420 bit mode.
     *
     * Available since NeuroPilot 8.0.3.
     */
    NEURON_AFBCTOYUV = 110,
...
/**
 * Sets an operand's shape hint for dynamic shape.
 *
 * Available since 8.0.0
 *
 * @param model The model to be modified.
 * @param index The index of the model operand we're setting.
 * @param dimensions The hinted dimensions.
 * @param dimensionCount The dimension count of the hinted dimensions.
 *
 * @return NEURON_NO_ERROR if successful.
 */
int NeuronModel_setOperandShapeHint(NeuronModel* model, int32_t index, const uint32_t* dimensions,
                                    uint32_t dimensionCount);
...
/**
 * Add an operation to a model.
 * The operands specified by inputs and outputs must be added by NeuronModel_addOperand before use.
 * Use NeuronOperationMeta to set the meta for this operation.
 *
 * Available since 8.0.0
 *
 * @param model The model to be modified.
 ...
 * @param outputCount The number of entries in the outputs array.
 * @param outputs An array of indexes identifying each operand.
 * @param meta The meta data configs set to the operation.
 *
 * @return NEURON_NO_ERROR if successful.
 */
int NeuronModel_addOperationWithMeta(NeuronModel* model, NeuronOperationType type,
                                     uint32_t inputCount, const uint32_t* inputs,
                                     uint32_t outputCount, const uint32_t* outputs,
                                     const NeuronOperationMeta* meta);
...
/**
 * Add an operation extension with meta data to a model.
 * The operands specified by inputs and outputs must be added by NeuronModel_addOperand before use.
 * User needs to specify the operation extension name and the desired device which executes the
 * operation extension. Use NeuronOperationMeta to set the meta for this operation.
 *
 * Available since 8.0.0
 *
 * @param model The model to be modified.
 ...
 * @return NEURON_NO_ERROR if successful.
 */
int NeuronModel_addOperationExtensionWithMeta(NeuronModel* model, const char* name,
...
/**
 * Sets the execution deadline associated with this execution. Required before calling
 * NeuronExecution_compute.
 *
 * Deadline: Execution deadline for the inference (in msec).
 * Setting any non-zero value would notify the scheduler that this inference is a real-time task.
 *
 * This default deadline should be zero, unless this inference is a real-time task.
 *
 * Available since 8.0.3
 *
 * @param execution The execution to be modified.
 * @param deadline The deadline for the inference (in msec).
 *
 * @return NEURON_NO_ERROR if successful.
 */
int NeuronExecution_setDeadline(NeuronExecution* execution, uint16_t deadline);

/**
 * Sets the execution abort time hint associated with this execution. Required before calling
 * NeuronExecution_compute.
 *
 * Execution abort time is the hint for the the maximum inference time for the execution (in msec).
 * If the inference is not completed before the abort time, the scheduler would abort the inference.
 *
 * This default abort time should be zero, unless users wish to abort the inference.
 *
 * Available since 8.0.3
 *
 * @param execution The execution to be modified.
 * @param boostValue The hint for the the maximum inference time for the execution (in msec).
 *
 * @return NEURON_NO_ERROR if successful.
 */
int NeuronExecution_setAbortTime(NeuronExecution* execution, uint16_t abortTime);
