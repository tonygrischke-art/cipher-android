...
class RoutineEngine(
    ...
    companion object {
        ...
        const val ACTION_ROUTINE_FIRED = "com.aetheria.cipher.ROUTINE_FIRED"
        const val ACTION_CHECK_TIME_ROUTINES = "com.aetheria.cipher.CHECK_TIME_ROUTINES"
    ...
    private fun registerReceivers() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
            context.registerReceiver(
                appChangeReceiver,
                IntentFilter(com.aetheria.cipher.accessibility.CipherAccessibilityService.ACTION_FOREGROUND_APP_CHANGED),
                android.content.Context.RECEIVER_NOT_EXPORTED
            )
        } else {
            @Suppress("UNSPECIFIED_REGISTER_RECEIVER_FLAG")
    ...
    private val appChangeReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            val packageName = intent?.getStringExtra(
                com.aetheria.cipher.accessibility.CipherAccessibilityService.EXTRA_PACKAGE_NAME
            ) ?: return
