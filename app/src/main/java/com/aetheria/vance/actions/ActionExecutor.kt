...
import com.aetheria.vance.shizuku.ShizukuNotAvailableException
...
class ActionExecutor(
    ...
    private fun handleNotificationAction(params: JSONObject): ActionResult {
        ...
        return when (action) {
            "dismiss" -> {
                try {
                    val intent = Intent("com.aetheria.vance.DISMISS_NOTIFICATION").apply {
            ...
            "read_last" -> {
                val intent = Intent("com.aetheria.vance.READ_NOTIFICATION").apply {
                    setPackage(context.packageName)
                }
                context.sendBroadcast(intent)
    ...
    private fun handleAccessibilityAction(params: JSONObject): ActionResult {
        ...
        return try {
            val intent = Intent("com.aetheria.vance.ACCESSIBILITY_ACTION").apply {
